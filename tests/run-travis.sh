...
case ${DEVICE} in
	...
	ios )
		...
		for file in *.dll; do
			~/iostools/Tools/OSX/mono-xcompiler --aot=full,asmonly,nodebug,static,outfile=$file.s $file || exit 1
		done
		mv *.s ../../Libraries || exit 1

		# build using xcode but do not sign
		cd ../../
		xcodebuild -alltargets -configuration Debug clean build CODE_SIGN_IDENTITY="" CODE_SIGNING_REQUIRED=NO || exit 1
		# fake sign
